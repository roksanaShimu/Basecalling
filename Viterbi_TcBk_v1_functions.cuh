...
__constant__ int32_t d_ev[4*number_of_events_per_kernel*2];
__constant__ int32_t d_pmax[2];
...
static inline void fixpt_traceback(vector<bcall_tcbk>& Bcall_TcBk, int Bcall_iter, int to, unsigned int j_max)
{
	...
	for (i = to - 1; i > from; --i) {
		...
		Bcall_TcBk[Bcall_iter].state_seq[i] = j_max;
		j_max = Bcall_TcBk[Bcall_iter].beta[i % TRACEBACK_LEN][j_max];
	}
...
__device__ int32_t dev_ln_ptransition(int32_t * devPtr_log_pr, unsigned j, int32_t *alpha, uint16_t *beta, unsigned alpha_read, unsigned counter, unsigned ith_tcbk, int number_of_events, int ith_event){
	...
	unsigned start_index=group_no*warp_size*21 +j%warp_size +ith_tcbk* N_STATES*21;
	...
	//if(threadIdx.x==2 && ith_event==1)printf("ith_event=%d ith_tcbk= %d j=%d log_p=%" PRIu32 " start_index= %d, devPtr_log_pr[start_index]=%" PRIu32 "  alpha[j+alpha_read*N_STATES]=%" PRIu32 "\n", ith_event, ith_tcbk, j, log_p, start_index, devPtr_log_pr[start_index], alpha[j+alpha_read*N_STATES] );


	//if(threadIdx.x==2 && ith_event==1)printf("ith_event=%d ith_tcbk= %d j=%d log_p=%" PRIu32 " start_index= %d, devPtr_log_pr[start_index]=%" PRIu32 "   j+alpha_read*N_STATES=%d  alpha[j+alpha_read*N_STATES]=%" PRIu32 "\n", ith_event, ith_tcbk, j, log_p, start_index, devPtr_log_pr[start_index], j+alpha_read*N_STATES, alpha[j+alpha_read*N_STATES] );
	...
	beta[j+ counter*N_STATES + ith_tcbk*N_STATES*number_of_events]=temp_beta;
	return log_p;
...
__device__ static inline int32_t dev_ln_pemission(int32_t *devPtr_pm, int32_t p_max,  int32_t ln_pt, unsigned j, unsigned k, unsigned ith_tcbk)
{


	//start index for devPtr_pm
	unsigned group_no=j/warp_size;
	unsigned start_index=group_no*warp_size*7 +j%warp_size + ith_tcbk* N_STATES*7;
